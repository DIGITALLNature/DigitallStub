...
public class FakeOrganizationService(TimeProvider timeProvider, FakeOrganizationServiceState state) : IOrganizationService
{
    ...
    {
        if (State.Relationships.ContainsKey(relationshipSchemaName))
        {
            return State.Relationships[relationshipSchemaName];
        }
    ...
    {
        foreach (var modelAssembly in State.ModelAssemblies)
        {
    ...
    public void ThrowIfNotKnownAttribute(string entityLogicalName, string attributeLogicalName)
    {
        ...
        if (!IsKnownAttributeForType(entityLogicalName, attributeLogicalName, out _))
        {
            ...
            if (!State.EntityMetadata.TryGetValue(entityLogicalName, out var entityMetadata) || entityMetadata.Attributes.All(a => a.LogicalName != attributeLogicalName))
            {
