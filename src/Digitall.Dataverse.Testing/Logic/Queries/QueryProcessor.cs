...
public class QueryProcessor
{
    ...
    public QueryExpression ConvertXmlDocumentToQueryExpression(XDocument xmlDocument)
    {
        ...
        var query = new QueryExpression(entityNode?.GetAttribute("name")?.Value) { ColumnSet = xmlDocument.ToColumnSet() };
    ...
    public static List<Entity> ProcessAggregateFetchXml(XDocument xmlDoc, List<Entity> internalResult)
    {
        ...
        foreach (var attr in xmlDoc.Descendants(ns + "attribute"))
        {
            ...
            if (attr.IsAttributeTrue("groupby"))
            {
                ...
                {
                    groups.Add(new SimpleValueGroup { OutputAlias = alias, Attribute = logicalName });
                }
            }
            else
            {
    ...
    private static List<Entity> ProcessGroupedAggregate(string entityName, IList<Entity> resultOfQuery, IList<FetchAggregate> aggregates, List<FetchGrouping> groups)
    {
        ...
        var grouped = resultOfQuery.GroupBy(e => { return groups.Select(g => g.Process(e)).ToArray(); }, new ArrayComparer());
