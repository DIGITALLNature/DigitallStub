...
internal class FetchProcessor(DataverseStub state)
{
    ...
    {
        //Validate nodes
        if (!xmlDocument.Descendants().All(el => XDocumentExtensions.IsFetchXmlNodeValid(el)))
    ...
    {
        return xmlDocument.Elements()   //fetch
            .Elements()     //entity
    ...
    public FilterExpression ExractFilterExpression(XElement elem)
    {
