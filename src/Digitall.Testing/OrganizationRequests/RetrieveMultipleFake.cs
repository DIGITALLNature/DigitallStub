...
// We instantiate a QueryExpression to be executed as we have the implementation done already
        ...
        private static void PatchDateFormat(Entity record, FakeOrganizationService organizationService)
        {
            if (organizationService.State.EntityMetadata.TryGetValue(record.LogicalName, out var entityMetadata))
            {
