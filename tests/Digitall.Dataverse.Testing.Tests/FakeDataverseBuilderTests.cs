...
public class FakeDataverseBuilderTests
{
    ...
    public void AddData_Should_BeRetrieved_From_OrganizationService()
    {
        ...
        var service = new FakeDataverseBuilder().AddData(entity).GetOrganizationService();
    ...
    {
        var service = new FakeDataverseBuilder().AddConfig("dg_TestFlag", "default", "override").GetOrganizationService();
    ...
    public void AddEntityMetadata_And_Relationships_Should_BeStored_In_Service()
    {
        ...
        typeof(EntityMetadata).GetProperty(nameof(EntityMetadata.ManyToManyRelationships))!.SetValue(accountMetadata, Array.Empty<ManyToManyRelationshipMetadata>());
        typeof(EntityMetadata).GetProperty(nameof(EntityMetadata.OneToManyRelationships))!.SetValue(accountMetadata, Array.Empty<OneToManyRelationshipMetadata>());
        typeof(EntityMetadata).GetProperty(nameof(EntityMetadata.ManyToOneRelationships))!.SetValue(accountMetadata, Array.Empty<OneToManyRelationshipMetadata>());

        var relationship = new OneToManyRelationshipMetadata { SchemaName = "dg_account_contact" };

        var service = new FakeDataverseBuilder().AddEntityMetadata(accountMetadata).AddRelationships(relationship).GetOrganizationService();

        service.State.EntityMetadata.Should().ContainKey("account");
        service.State.Relationships.Should().ContainKey("dg_account_contact");
    }
}
